...
Opt in with ``VLM_BACKEND=moondream`` and the ``moondream`` extra. It implements
only ``describe`` (no ``embed``), so pair it with the ``single`` or ``uniform``
selector; the factory rejects ``clusters`` for any backend that cannot embed.
